...
PACKAGE_URL = 'git+https://github.com/IBM/iot-analytics-anomaly@transformer'
...
class TestTransformer(BaseTransformer):
    ...
    def execute(self, df, start_ts = None,end_ts=None):
        ...
        df[self.output_items] = 5 #df[self.input_items]
        return df
        ...
        logging.debug( results['values'][-1] )
        df[self.input_items[0]] = np.zeros(len(df))
        df[self.output_items[0]] = df[self.input_items[0]]
